...
public class Parser {
	...
	public AstNode parseStmt() throws SyntaxErrorException 
	{
		...
		else if (token.getKlazz() == Token.IF) {
			stmt = this.parseIfElseStatement();
		}
		else if (token.getKlazz() == Token.WHILE) {
	...
	public AstNode parseWhileStatement() throws SyntaxErrorException
	{
		Token tw = this.tokenizer.peekToken();
		if (tw.getKlazz() != Token.WHILE) {
			throw new SyntaxErrorException("expecting a 'while' token, but found :" + tw);
		}
		...
		AstNode loopBody = this.parseStmt();
				
		return new AstWhile(tw, cond, loopBody);
	}
	 ...
	 * @throws SyntaxErrorException
	 */
	public AstNode parseIfElseStatement() throws SyntaxErrorException
	{
		Token tif = this.tokenizer.peekToken();
		if (tif.getKlazz() != Token.IF) {
			throw new SyntaxErrorException("expecting a 'if' token, but found :" + tif);
		}
		this.tokenizer.consumeToken();
		...
		AstNode thenClause = this.parseStmt();
		
		t = this.tokenizer.peekToken();
		if (t.getKlazz() != Token.ELSE) {
			return new AstIfElse(tif, cond, thenClause, null);
		}
		this.tokenizer.consumeToken();
		
		AstNode elseClause = this.parseStmt();
		return new AstIfElse(tif, cond, thenClause, elseClause);
	}
